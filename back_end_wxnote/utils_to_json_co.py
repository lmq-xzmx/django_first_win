...
def DataToJson_list():
    try:
        ...
        # 去除首尾的中括号
        return json_list_datar

if __name__ == '__main__':
    # 调用函数
    json_group_data = DataToJson_group()
    json_list_data  = DataToJson_list()
    json_co_data = []
    json_list_data=json.loads(json_list_data)
    json_group_data=json.loads(json_group_data)
    
    
    print("json_list_data的type::::::::",type(json_list_data))
    print("len(json_group_data)::::::",len(json_group_data))
    print("len(json_list_data)::::::",len(json_list_data))
    for i in range(0,len(json_group_data)):
        for o in range(0,len(json_list_data)):
            print("json_list_data[o]:",json_list_data[o])
            print("json_group_data[i]:",json_group_data[i])
            if json_list_data[o]["e_note_list_id"]==json_group_data[i]["id"]:   
                print("它俩相等")
                result = {}
                result[0]=json_list_data[o]['e_note_list_id']
                #print("print(result):",result)
                #print("result[0]:",result[0])
                result[1]=json_group_data[i]['date']
                #print("print(result):",result)
                #print("result[1]:",result[1])
                result[2]=json_group_data[i]['custom_topic']
                result[3]=json_group_data[i]['mail_title']
                result[4]=str(json_group_data[i]['created'])
                result[5]=json_list_data[o]['id']
                result[6]=str(json_list_data[o]['talk_time'])
                result[7]=json_list_data[o]['chater']
                result[8]=json_list_data[o]['chat_content']
                print("print(result):",result)
                json_co_data.append(result)        
    
    print(u'转换为json格式的数据：', json_co_data)

    # 以读写方式w+打开文件，路径前加r，防止字符转义
    f = open("./html/json_co_data.json", "w")
    # 写数据 # 去除首尾的中括号
    json_co_datar = json.dumps(json_co_data, ensure_ascii=False)
    print("type(json_co_datar):::::::",type(json_co_datar))
    print("json_co_datar:::::",json_co_datar)
    
    json_co_datar = json_co_datar[1:len(json_co_datar) - 1]

    f.write(json_co_datar)
    # 关闭文件
    f.close()
